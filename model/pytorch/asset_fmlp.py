...
pc_tv_indices = train_data[mask].index
non_pc_tv_indices = train_data[~mask].index
...
data_with_pc_tv = train_data.loc[pc_tv_indices].reset_index(drop=True)
data_without_pc_tv = train_data.loc[non_pc_tv_indices].reset_index(drop=True)

# Dataset 객체 생성
train_dataset = PandasDataset(data_without_pc_tv)
val_dataset = PandasDataset(data_with_pc_tv)
...
for epoch in tqdm(range(epochs)):
    ...
    print(f"Epoch {epoch+1}/{epochs}, Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, Test F1 score : {f1_score}")

# Prediction

torch.save(model.state_dict(), "fmlp_reloop_without_pctv.pth")
